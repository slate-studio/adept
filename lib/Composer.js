...
class Composer {
  constructor({ title, version, host, basePath, tags, modules, schemes }) {
    ...
    for (const module of modules) {
      ...
      // TODO: This might not be required anymore as all definitions are pulled
      //       from operations and referenced in there.
    ...
    for (const definition of this._definitions) {
      ...
      // NOTE: This workaround for multiple version of the same component,
      //       the one with properties.id would dominate.
      const isAlreadyDefined = this._spec.definitions[definition.id]
      const hasPropertiesId  = !!schema.properties && !!schema.properties.id

      if (!isAlreadyDefined || hasPropertiesId) {
        delete schema.id
